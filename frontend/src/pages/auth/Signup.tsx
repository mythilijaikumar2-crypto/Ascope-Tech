      ...
      >
        <div className="text-center mb-12">
          <Link to="/" className="inline-flex items-center gap-3 mb-8 group">
            <img src="/src/assets/ascopetech_logo.png" alt="Ascope Tech Logo" className="w-40 h-40 object-contain group-hover:scale-110 transition-transform drop-shadow-lg" />
          </Link>
        ...
        <form onSubmit={handleSignup} className="space-y-6">
          ...
          {status.message && (
            <div className={`p-4 rounded-xl text-sm font-bold text-center ${status.type === 'error' ? 'bg-red-50 text-red-600' : status.type === 'success' ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600'}`}>
              {status.message}
            </div>
          )}

          <GradientButton type="submit" className="w-full py-5 text-xl mt-4">
