...
const Footer: React.FC = () => {
  return (
    <footer className="bg-navy text-white pt-16 pb-10 overflow-hidden relative border-t border-white/5">
      {/* Background circles */}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-12 md:gap-8 lg:gap-12">
          {/* Brand - 4 columns */}
          <div className="lg:col-span-4 space-y-8">
            <Link to="/" className="inline-block group">
              <div className="flex flex-col items-start">
                <motion.img 
                  ...
                  className="w-24 h-24 object-contain drop-shadow-[0_0_15px_rgba(23,181,231,0.2)] mb-0" 
                />
                <span className="text-2xl font-heading font-black text-white tracking-tight -mt-4">
                  Ascope <span className="text-accent">Tech</span>
                </span>
              </div>
            </Link>
            <p className="text-white/60 leading-relaxed text-sm max-w-sm font-body">
            ...
            <div className="flex gap-4">
              {[FacebookIcon, TwitterIcon, InstagramIcon, LinkedinIcon].map((Icon, idx) => (
                <a key={idx} href="#" className="w-11 h-11 rounded-xl bg-white/5 flex items-center justify-center text-white hover:bg-accent hover:text-white hover:scale-110 transition-all border border-white/10 group">
                  <Icon size={20} />
                </a>
              ))}
            </div>
          ...
          <div className="lg:col-span-2">
            <h4 className="font-heading font-black text-white mb-8 border-l-4 border-accent pl-4 uppercase tracking-widest text-[11px]">Company</h4>
            <ul className="space-y-4">
              {['About Us', 'Courses', 'Trainers', 'Placements', 'Contact'].map((link) => (
                <li key={link}>
                  <Link to={`/${link.toLowerCase().replace(' ', '-')}`} className="text-white/60 hover:text-accent hover:translate-x-1 transition-all flex items-center gap-2 font-bold text-sm group">
                    <span className="w-1 h-1 rounded-full bg-accent/30 group-hover:bg-accent transition-colors" /> {link}
                  </Link>
                </li>
              ))}
            </ul>
          ...
          {/* Support - 2 columns */}
          <div className="lg:col-span-2">
            <h4 className="font-heading font-black text-white mb-8 border-l-4 border-accent pl-4 uppercase tracking-widest text-[11px]">Categories</h4>
            <ul className="space-y-4">
              {['Full Stack Dev', 'Data Science', 'UI/UX Design', 'Cloud Computing', 'Cybersecurity'].map((item) => (
                <li key={item}>
                  <a href="#" className="text-white/60 hover:text-accent hover:translate-x-1 transition-all flex items-center gap-2 font-bold text-sm group">
                    <span className="w-1 h-1 rounded-full bg-accent/30 group-hover:bg-accent transition-colors" /> {item}
                  </a>
                </li>
              ))}
            </ul>
          ...
          <div className="lg:col-span-4">
            <h4 className="font-heading font-black text-white mb-8 border-l-4 border-accent pl-4 uppercase tracking-widest text-[11px]">Contact Info</h4>
            <div className="space-y-6">
              <div className="flex gap-4 text-white/60 items-start group">
                <div className="w-11 h-11 rounded-xl bg-white/5 flex items-center justify-center text-accent shrink-0 border border-white/5 group-hover:border-accent/30 transition-all">
                   <MapPinIcon size={20} />
                </div>
                <div className="flex flex-col">
                  <span className="text-[11px] font-black uppercase tracking-wider text-white/40 mb-1">Location</span>
                  <span className="text-sm leading-relaxed">123 Tech Avenue, Silicon Valley, <br />California, CA 94025</span>
                </div>
              </div>
              <div className="flex gap-4 text-white/60 items-center group">
                <div className="w-11 h-11 rounded-xl bg-white/5 flex items-center justify-center text-accent shrink-0 border border-white/5 group-hover:border-accent/30 transition-all">
                   <PhoneIcon size={20} />
                </div>
                <div className="flex flex-col">
                  <span className="text-[11px] font-black uppercase tracking-wider text-white/40 mb-1">Phone</span>
              ...
              <div className="flex gap-4 text-white/60 items-center group">
                ...
                <div className="flex flex-col">
                  <span className="text-[11px] font-black uppercase tracking-wider text-white/40 mb-1">Email</span>
